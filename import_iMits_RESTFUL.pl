...
my ($phenotyping_arg, $colony_arg, $start_page, $last_page, $filter, $debug, $help) = (0, undef, 1, undef, '', 0, 0);
...
$mgi_accession_id=$decoded->{"mgi_accession_id"};
		...
		print "Colony name:        $colony_prefix\n";
		...
		if ($phenotyping_arg) {
		    ...
		    print "MI Attempt Colony:  $colony_prefix\n";
		}
		...
		if ($is_active eq 'false') {
		    ...
		    print "Project not active anymore - Check EMMA distribution then skip\n";
		...
		if (!is_emma_repository($repository)) {
		    print "$repository is not an EMMA production node\n";
		    next;
		}
...
print "ABOUT TO UPDATE CONSORTIUM INFO\n\n" if ($debug);
...
print "IMPC DATA IS AVAILABLE\n\n" if ($debug);
		...
		if ($genotype_confirmed || ($phenotyping_arg && $mouse_allele_type =~ /^[bcde]/)) {

		    # if there are distribution centers, then loop through and look for the EMMA network

		    if (scalar(@{$distribution_info}) > 0) {
			...
			for (my $i = 0; $i < scalar(@{$distribution_info}); $i++) {
				...
				print "Deposited material: " . $distribution_info->[$i]->{"deposited_material_name"} . "\n" if ($debug);
...
print "URL Allele3:      $urlallele\n";
			...
			if ($allele =~ /Wtsi/) {
			    ...
			    print STDERR "ERROR: The allele nomenclature $allele is not valid for this script - PLEASE FIX THIS - and restart the script";
			    exit 1;
			}
			...
			print "Full allele name:   " . $full_allele_name . "\n" if ($debug);		    
...
print "IMPC DATA HAS BEEN UPDATED AT LINE 1268 colony prefix is $colony_name\n\nIMPC data availability is $impc_data_available\n\npheno attempts is $phenotype_attempts\n\n" if ($debug);
	...
	print "CREATE/UPDATE GENE, ALLELE AND MUTATION RECORDS\n" if ($debug);
	...
	if (!$id_gene){
	    ...
	    &execute_query($sql, $emma_dbh);
	    print "Gene entry didn't exists for strain $id_str: new marker symbol inserted $marker_symbol\n" if ($debug);
	}
	($id_gene) = &execute_query("SELECT id_gene FROM genes WHERE symbol ='$marker_symbol'", $emma_dbh);
	print "Gene $marker_symbol id is $id_gene\n" if ($debug);
	...
	print "SELECT id_allel FROM alleles WHERE alls_form = '$allele'\n" if ($debug);
	...
	if (!$id_allele && $id_gene){
	    ...
	    print "Allele entry didn't exists for strain $id_str: new allele symbol inserted $allele\n" if ($debug);
	}
	print "Allele $allele id is $id_allele\n" if ($debug);
	...
	if ($id_str){# to exclude UCD lines and BCM lines
		...
		print STDERR $mutation_types[1] . " " . $strain_name . "\n" if ($mutation_types[1] eq 'KOF' && $debug);
		...
		if (!$mut_id) {
		    print "Mutation does not exist for this gene will create one\n" if ($debug);
		...
		if ($mutation_types[1] eq 'KOF') {
		    print STDERR $mutation_types[1]. " update mutation " .$strain_name . "\n" if ($debug);
    ...
    print "=== EMMA_UPDATE PROCEDURE END ===\n" if ($debug);
}
...
print STDERR "Setting IMPC data - Colony prefix arg= $colony_prefix and IMPC data availability is set to $impc_data_available \n" if ($debug);
    if (defined($colony_prefix) && length($colony_prefix) > 0 && $impc_data_available == 1) {
	print STDERR "Marking $colony_prefix as having IMPC data\n" if ($debug);
...
sub update_strain_consortium_information {
	...
	print STDERR "Marking $colony_prefix as IMPC strain\n" if ($debug);
...
sub check_existing_strain{
    ...
    if ($debug) {
	print "\tcheck_existing_strain:\n";
	print "\t\tStrain name: $strain_name\n";
	print "\t\tEPD ID: $epd_id\n";
	print "\t\tColony name: $colony_name\n";
	print "\t\tMI date: $mi_date\n";
	print "\t\tid_labo: $id_labo\n";
	...
	if (defined($colony_name) && length($colony_name) > 0 ) {
	    ...
	    $id_str = $id_q;
	    if ($id_str) {
		print STDERR "check_existing_strain ($b_same_lab): strain $id_str for colony prefix $colony_name - $strain_name - $epd_id - $mi_date - $id_labo ($labo_code) found.\n" if ($debug);
		return $id_str;
	    }
	} 
	...
	if (!$b_strict_colony_name) {
	    ...
	    $id_str = $id_q;
	    print STDERR "check_existing_strain ($b_same_lab): strain $id_str (no colony prefix) - $strain_name - $epd_id - $mi_date - $id_labo ($labo_code) found.\n" if ($id_str && $debug);
	}
    }
...
sub reject_strain_access_in_emma {
	...
	if ($id_str && $str_access ne 'R') {
	    print STDERR "Reject_strain_access:\t$colony_prefix\t$id_str\t$name\t$str_access\n";
	    $sql = "UPDATE strains set str_access = 'R' WHERE id_str ='$id_str'";
	    &execute_query($sql, $emma_dbh);
	    $rejected = 1;
	}
...
print "we are at l1576 rule 15" if $debug;
	...
	print STDERR "STOCK nomenclature for:\t" if ($debug);
	print STDERR "ES Cell:[$escell_strain]\tTest Cross:[$test_cross]\tBack Cross:[$back_cross]\n" if ($debug);
	$countStock++;
    ...
    if ($debug) {
	print STDERR "$rule_number\tES CELL\tTEST CROSS\tBACK CROSS\n";
	print STDERR "\t[$escell_strain]\t[$test_cross]\t[$back_cross]\n";
	print STDERR $strain_name . "\n";
    }
...
sub compute_mta_file {
    my ($pipeline, $repository, $distribution_center, $consortium, $phenotyping_attempt) = @_;
